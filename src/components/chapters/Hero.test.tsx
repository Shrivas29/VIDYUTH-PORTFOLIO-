...
describe("headlineDelayBase", () => {
  it("returns the 2.5s post-splash delay when the splash was shown this load", () => {
    expect(headlineDelayBase(true)).toBe(2.5);
  });
...
describe("Hero headline entrance wiring", () => {
  ...
  it("schedules the entrance at the 2.5s post-splash delay (+ stagger) when the splash was shown this load", () => {
    vi.mocked(splashWasShownThisLoad).mockReturnValue(true);
    render(<Hero />);

    const words = screen.getAllByTestId("word");
    expect(words[0]).toHaveAttribute("data-delay", "2.5");
    expect(words[1]).toHaveAttribute("data-delay", String(2.5 + 0.12));
    expect(words[2]).toHaveAttribute("data-delay", String(2.5 + 2 * 0.12));
  });
